...
// AT Mode
//*******************************************************
// Identical to the ESP version, except that the settings are stored with the EEPROM
// backed Preferences replacement, the restart is a RP watchdog reboot, and
// AT+PROTOCOL=6 (ESP-NOW) and AT+WIFICHANNEL=13 are rejected, since the CYW43439 can do
// neither ESP-NOW nor channel 13.
//*******************************************************
    ...
    if (avail > 0) {
        ...
        for (int i = 0; i < AT_CMDS_NUM; i++) {
            ...
            if (strcmp(at_buf, at_cmd) == 0) {
                ...
                if (i == AT_WIFICHANNEL_13) {
                    SERIAL.write("KO\r\n"); // cmd recognized, but the Pico W CLM blob has no channel 13
                } else
                if (i >= AT_WIFICHANNEL_1 && i <= AT_WIFICHANNEL_11) {
                    at_buf[0] = 'O';
                    at_buf[1] = 'K';
                ...
                if (i == AT_PROTOCOL_6_ESPNOW) {
                    SERIAL.write("KO\r\n"); // cmd recognized, but ESP-NOW is not available on a CYW43439
                } else
