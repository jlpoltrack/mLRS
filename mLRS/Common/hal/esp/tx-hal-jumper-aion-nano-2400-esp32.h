...
#define KEY_RIGHT_THRESH          70 // 3600 // tom = ELRS: 3511, ow: 3712
#define KEY_UP_THRESH             870 // 2800 // tom = ELRS: 2839, ow: 2966, marc: 2800 didn't work for him. 2839 did
#define KEY_DOWN_THRESH           600 // 2200 // tom = ELRS: 2191, ow: 2282
#define KEY_LEFT_THRESH           230 // 1650 // tom = ELRS: 1616, ow: 1685
...
IRAM_ATTR uint8_t fiveway_read(void)
{
    ...
    if (adc > (KEY_CENTER_THRESH-100) && adc < (KEY_CENTER_THRESH+30)) return (1 << KEY_CENTER);
    if (adc > (KEY_LEFT_THRESH-100) && adc < (KEY_LEFT_THRESH+100)) return (1 << KEY_LEFT); 
    if (adc > (KEY_DOWN_THRESH-100) && adc < (KEY_DOWN_THRESH+100)) return (1 << KEY_DOWN);
    if (adc > (KEY_UP_THRESH-100) && adc < (KEY_UP_THRESH+100)) return (1 << KEY_UP);
    if (adc > (KEY_RIGHT_THRESH-30) && adc < (KEY_RIGHT_THRESH+100)) return (1 << KEY_RIGHT);
    return 0;
}
#endif
...
void ser_or_com_init(void)
{
    ...
    for (uint8_t i = 0; i < 16; i++) {
        ...
        if (adc > (KEY_DOWN_THRESH-150) && adc < (KEY_DOWN_THRESH+150)) cnt++;
    }
...
#define POWER_GAIN_DBM            28 // 28 // gain of a PA stage if present
