...
//-------------------------------------------------------
// TX DIY WeAct STM32H503Cx CoreBoard + E22 (SX126x), 915 MHz FCC
//-------------------------------------------------------
...
//
// This is the Tx counterpart of rx-hal-diy-WeAct-E22-h503cb.h. Radio wiring, LED and
// button are identical to the rx target, so the same hardware can be flashed either way.
//
...
//   PA11/PA12   USB DN/DP, wired to the Type-C connector, used here for the Com port
...
//
//
// Wiring to the E22-900M / SX1262 module, same as on the rx target:
...
//
// Wiring to the radio:
//   JR bay pin5 -> PB8    (USART3_RX, AF13), also serves as the IN pin
//   serial Tx   -> PA9    (USART1_TX, AF7)
//   serial Rx   -> PA10   (USART1_RX, AF7)
//   debug Tx    -> PB6    (LPUART1_TX, AF8)
//
// The JR pin5 signal sits on the USART3 *Rx* pin and is driven by the peripheral's own
// invert + swap, so it needs no diode and no external inverter. PB3 is USART3_TX and stays
// unused, it is held as an input.
//
...
// Sectors 14 and 15 are the emulated eeprom, i.e. 0x0801C000 .. 0x0801FFFF is reserved and
// the firmware must stay below 112 kB. The rx target additionally reserves a sector for the
// powerup counter, but that is a CommonRx feature, so a Tx gets that sector back.
#define EE_START_PAGE             14 // 128 kB flash, 8 kB sector
...
// com is on USB, see DEVICE_HAS_COM_ON_USB, so no UARTC

#define UART_USE_UART3_PB3PB8 // JR pin5, MBridge // PB3 = Tx, PB8 = Rx, both AF13
...
#define JRPIN5_FULL_INTERNAL_ON_RX // does not require an external diode, signal is on PB8

// the in port shares the pin5 pin, see DEVICE_HAS_IN_ON_JRPIN5_RX, so no UARTE
...
//-- In port
// the in port shares the pin on the JR bay pin5, so it is fully handled by in_interface.h,
// no in_init_gpio(), in_set_normal(), in_set_inverted() are needed here
...
uint32_t porta[] = {
    LL_GPIO_PIN_1, LL_GPIO_PIN_2, LL_GPIO_PIN_3, LL_GPIO_PIN_8, LL_GPIO_PIN_15,
};

uint32_t portb[] = {
    LL_GPIO_PIN_0, LL_GPIO_PIN_1, LL_GPIO_PIN_2, LL_GPIO_PIN_3, LL_GPIO_PIN_4, LL_GPIO_PIN_5,
    LL_GPIO_PIN_8, LL_GPIO_PIN_10, LL_GPIO_PIN_12, LL_GPIO_PIN_13, LL_GPIO_PIN_14, LL_GPIO_PIN_15,
};
