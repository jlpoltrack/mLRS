...
// no DIO1, so the FSK FifoLevel is polled every 1 ms on CC2 of the micros timer, which has no other isr
#define DEVICE_HAS_SX127x_FSK
#define SX_FIFO_POLL_PERIOD_US        1000
#define SX_DIO1_EXTI_IRQHandler       TIM3_IRQHandler
...
// only called in FSK mode, the poll must have the DIO isr priority, so the two spi users can't preempt each other
