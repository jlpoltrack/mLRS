...
// or on devices without DIO1 by a 1 ms timer isr polling FifoLevel
...
class Sx127xDriverCommon : public Sx127xDriverBase
{
  public:
    ...
    bool IsInLoraMode(void) { return is_in_lora_mode; }
    ...
    void ResetToLoraConfiguration(tSxGlobalConfig* const _gconfig)
    {
        ...
        if (!is_in_lora_mode) { // we need to switch from FSK to LoRa, power is kept, the PA registers are shared
            _configure_lora();
            return;
        }
...
/*        uint8_t rxStartBufferPointer;
    ...
    void SendFrame(uint8_t* const data, uint8_t len, uint16_t tmo_ms) // SX1276 doesn't have a Tx timeout
    {
        if (!is_in_lora_mode) {
            ...
            // fsk_state is changed only outside of spi accesses, so a FIFO poll isr can't interleave its spi
            fsk_state = FSK_STATE_IDLE;
            SetStandby();
            _clear_fifo_fsk();
...
//XX        delay_us(1000); // is this needed ????
...
#ifdef DEVICE_HAS_SX127x_FSK
        if (!IsInLoraMode()) sx_dio1_enable_exti_isr(); // DIO1 is only needed in FSK, avoids the poll timer in LoRa
#endif
    }
    ...
    void ResetToLoraConfiguration(tSxGlobalConfig* const _gconfig)
    {
        ...
        sx_dio1_init_exti_isroff();
#endif
        Sx127xDriverCommon::ResetToLoraConfiguration(_gconfig);
    }
...
#ifdef DEVICE_HAS_SX127x_FSK
        if (!IsInLoraMode()) sx2_dio1_enable_exti_isr(); // DIO1 is only needed in FSK
#endif
    }
