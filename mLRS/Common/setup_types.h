...
typedef enum : uint8_t {
  ...
  SETUP_FREQUENCY_BAND_868_MHZ_PLUS_2P4_GHZ, // Dual-band: 868 on SX1, 2.4 on
                                             // SX2
  SETUP_FREQUENCY_BAND_915_MHZ_FCC_PLUS_2P4_GHZ, // Dual-band: 915 on SX1, 2.4
                                                 // on SX2
...
SX_FHSS_CONFIG_FREQUENCY_BAND_ENUM
cvt_to_sx_fhss_frequency_band(uint8_t setup_frequency_band);
SETUP_FREQUENCY_BAND_ENUM
cvt_to_setup_frequency_band(uint8_t sx_fhss_frequency_band);
...
typedef enum {
  ...
  DIVERSITY_R_ENABLED_T_ANTENNA1, // receive diversity enabled, transmit antenna
                                  // 1
  DIVERSITY_R_ENABLED_T_ANTENNA2, // receive diversity enabled, transmit antenna
                                  // 2
  DIVERSITY_NUM,
} DIVERSITY_ENUM;
...
#else
#define SERIAL_LINK_MODE_IS_MAVLINK(x)                                         \
  ((x) == SERIAL_LINK_MODE_MAVLINK || (x) == SERIAL_LINK_MODE_MAVLINK_X)
...
typedef struct {
  ...
  // for transmitters this is populated upon first connection, see
  // SetupMetaData.rx_available mechanism
  tRxSetup Rx;
  ...
  // parameters common to both Tx and Rx
  // cannot be changed on the fly, loss of connection will happen, needs
  // restart/reconnect
...
typedef struct {
  ...
  uint8_t Ortho;
  uint8_t Except;
  uint16_t FrequencyBand_allowed_mask; // copy of SetupMetaData for sx1, is
                                       // modified for sx2
} tFhssGlobalConfig;
