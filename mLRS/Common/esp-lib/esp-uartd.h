...
#else
    ESP_ERROR_CHECK(uart_set_pin(UARTD_SERIAL_NO, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));  // Fix this
#endif

    ESP_ERROR_CHECK(uart_driver_install(UARTD_SERIAL_NO, UARTD_RXBUFSIZE, UARTD_TXBUFSIZE, 0, NULL, 0));  // Rx Buf Size needs to be > 128
    ESP_ERROR_CHECK(uart_set_rx_full_threshold(UARTD_SERIAL_NO, 8));
    ESP_ERROR_CHECK(uart_set_rx_timeout(UARTD_SERIAL_NO, 1));
