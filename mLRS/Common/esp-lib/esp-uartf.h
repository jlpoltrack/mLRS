...
#else
    ESP_ERROR_CHECK(uart_set_pin(UARTF_SERIAL_NO, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE));  // Fix this
#endif

    ESP_ERROR_CHECK(uart_driver_install(UARTF_SERIAL_NO, UARTF_RXBUFSIZE, UARTF_TXBUFSIZE, 0, NULL, 0));  // Rx Buf Size needs to be > 128
    ESP_ERROR_CHECK(uart_set_rx_full_threshold(UARTF_SERIAL_NO, 8));
    ESP_ERROR_CHECK(uart_set_rx_timeout(UARTF_SERIAL_NO, 1));
