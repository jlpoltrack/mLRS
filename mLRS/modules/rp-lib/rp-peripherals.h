...
#define IO_MODE_Z                 MODE_GPIO_INPUT
#define IO_MODE_INPUT_ANALOG      MODE_GPIO_INPUT
...
inline void gpio_init(uint8_t pin, IOMODEENUM mode) {
    if (mode == MODE_GPIO_INPUT || mode == MODE_ANALOG) {
        // MODE_ANALOG is a plain "don't care" input, same as on esp. adc pins are
        // set up by analogRead() itself, and disabling the pad here would break the
        // pull-up that a resistor-ladder fiveway needs
        pinMode(pin, INPUT);
