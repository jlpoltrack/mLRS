...
inline uint16_t uartd_putc(char c)
{
    ...
    if ((uartd_txwritepos == uartd_txreadpos) && !(hw->fr & UART_UARTFR_TXFF_BITS)) {
        hw->dr = (uint8_t)c;
        return 1;
    }
...
inline void uartd_tx_flush(void)
{
    ...
    while (uart_get_hw(UARTD_UART_INST)->fr & UART_UARTFR_BUSY_BITS) {}
}
...
void _uartd_initit(uint32_t baud, UARTPARITYENUM parity, UARTSTOPBITENUM stopbits)
{
    ...
    UARTD_SERIAL_NO.ignoreFlowControl();  // send USB CDC data regardless of DTR
}
