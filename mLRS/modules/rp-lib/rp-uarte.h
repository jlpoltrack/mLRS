...
inline uint16_t uarte_putc(char c)
{
    ...
    if ((uarte_txwritepos == uarte_txreadpos) && !(hw->fr & UART_UARTFR_TXFF_BITS)) {
        hw->dr = (uint8_t)c;
        return 1;
    }
...
inline void uarte_tx_flush(void)
{
    ...
    while (uart_get_hw(UARTE_UART_INST)->fr & UART_UARTFR_BUSY_BITS) {}
}
...
void _uarte_initit(uint32_t baud, UARTPARITYENUM parity, UARTSTOPBITENUM stopbits)
{
    ...
    UARTE_SERIAL_NO.ignoreFlowControl();  // send USB CDC data regardless of DTR
}
