...
#define I2C_DMA_CMD_BUF_SIZE  1033  // 8 cmdhome preamble + 1 reg + 1024 data
...
// no initializer here, it would move cmd_buffer's 2 kB of zeros into the flash image
static i2c_dma_state_t i2c_dma;
static bool i2c_dma_initialized = false;
...
// async DMA transfer with cmdhome command preamble
// formats a single DMA buffer: cmdhome commands (RESTART, no STOP) followed
// by data write (RESTART + STOP). avoids blocking Wire call for cmdhome.
...
extern "C" HAL_StatusTypeDef i2c_put_with_cmdhome(uint8_t reg_adr, uint8_t* buf, uint16_t len)
{
    ...
    // cmdhome command bytes: set column address 0-127, page address 0-7
    static const uint8_t cmdhome[] = { 0x21, 0, 127, 0x22, 0, 7 };

    // format cmdhome as two 3-byte command sub-transactions via register 0x00
    // use RESTART between sub-transactions, no intermediate STOP
    uint16_t idx = 0;
    for (uint8_t s = 0; s < 6; s += 3) {
        i2c_dma.cmd_buffer[idx++] = (uint16_t)0x00 | I2C_IC_DATA_CMD_RESTART_BITS;
        i2c_dma.cmd_buffer[idx++] = (uint16_t)cmdhome[s];
        i2c_dma.cmd_buffer[idx++] = (uint16_t)cmdhome[s + 1];
        i2c_dma.cmd_buffer[idx++] = (uint16_t)cmdhome[s + 2];
    }

    // sub-tx 3: data register + framebuffer
    i2c_dma.cmd_buffer[idx++] = (uint16_t)reg_adr | I2C_IC_DATA_CMD_RESTART_BITS;
    for (uint16_t i = 0; i < len - 1; i++) {
        i2c_dma.cmd_buffer[idx++] = (uint16_t)buf[i];
    }
    i2c_dma.cmd_buffer[idx++] = (uint16_t)buf[len - 1] | I2C_IC_DATA_CMD_STOP_BITS;

    if (!i2c_dma_start(idx)) return HAL_BUSY;
