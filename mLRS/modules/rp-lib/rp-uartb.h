...
inline uint16_t uartb_putc(char c)
{
    ...
    if ((uartb_txwritepos == uartb_txreadpos) && !(hw->fr & UART_UARTFR_TXFF_BITS)) {
        hw->dr = (uint8_t)c;
        return 1;
    }
...
inline void uartb_tx_flush(void)
{
    ...
    while (uart_get_hw(UARTB_UART_INST)->fr & UART_UARTFR_BUSY_BITS) {}
}
...
void _uartb_initit(uint32_t baud, UARTPARITYENUM parity, UARTSTOPBITENUM stopbits)
{
    ...
    UARTB_SERIAL_NO.ignoreFlowControl();  // send USB CDC data regardless of DTR
}
