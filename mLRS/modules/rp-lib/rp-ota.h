...
class tRpOta
{
  ...
  private:
    ...
    //
    // NOTE: core 0 is idled for the whole erase+program (tens of ms), so the can2040 IRQ is
    // dead and every CAN frame on the bus in that window is lost. this is safe only because
    // of the caller's sequencing: the write happens while handling a file.Read response, and
    // the next request is not sent until afterwards, so no response we depend on is in flight
    // during the stall. do not pipeline (request next chunk before writing the current one)
