...
inline uint16_t uartf_putc(char c)
{
    ...
    if ((uartf_txwritepos == uartf_txreadpos) && !(hw->fr & UART_UARTFR_TXFF_BITS)) {
        hw->dr = (uint8_t)c;
        return 1;
    }
...
inline void uartf_tx_flush(void)
{
    ...
    while (uart_get_hw(UARTF_UART_INST)->fr & UART_UARTFR_BUSY_BITS) {}
}
...
void _uartf_initit(uint32_t baud, UARTPARITYENUM parity, UARTSTOPBITENUM stopbits)
{
    ...
    UARTF_SERIAL_NO.ignoreFlowControl();  // send USB CDC data regardless of DTR
}
