...
inline uint16_t uart$_putc(char c)
{
    ...
    if ((uart$_txwritepos == uart$_txreadpos) && !(hw->fr & UART_UARTFR_TXFF_BITS)) {
        hw->dr = (uint8_t)c;
        return 1;
    }
...
inline void uart$_tx_flush(void)
{
    ...
    while (uart_get_hw(UART$_UART_INST)->fr & UART_UARTFR_BUSY_BITS) {}
}
...
void _uart$_initit(uint32_t baud, UARTPARITYENUM parity, UARTSTOPBITENUM stopbits)
{
    ...
    UART$_SERIAL_NO.ignoreFlowControl();  // send USB CDC data regardless of DTR
}
