...
inline uint16_t uart_putc(char c)
{
    ...
    if ((uart_txwritepos == uart_txreadpos) && !(hw->fr & UART_UARTFR_TXFF_BITS)) {
        hw->dr = (uint8_t)c;
        return 1;
    }
...
inline void uart_tx_flush(void)
{
    ...
    while (uart_get_hw(UART_UART_INST)->fr & UART_UARTFR_BUSY_BITS) {}
}
...
void _uart_initit(uint32_t baud, UARTPARITYENUM parity, UARTSTOPBITENUM stopbits)
{
    ...
    UART_SERIAL_NO.ignoreFlowControl();  // send USB CDC data regardless of DTR
}
