...
void can_init(bool canfd)
{
    (void)canfd; // can2040 is classic CAN only, rp boards declare DEVICE_HAS_DRONECAN
    ...
    int16_t res = dc_hal_compute_timings(clock_get_hz(clk_sys), 1000000, &timings);
    if (res < 0) return;
