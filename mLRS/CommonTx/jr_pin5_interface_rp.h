...
// tx staging buffer size (not from HAL, this UART is PIO-driven)
#define UART_TXBUFSIZE            512

// pio resources - use PIO0, state machines are dynamically claimed to avoid init order dependencies
...
// tx staging buffer and IRQ-driven transmit state
static uint8_t tx_buf[UART_TXBUFSIZE];
...
void pio_uart_irq_handler(void) {
    ...
    // handle RX: parse bytes directly from PIO FIFO (no intermediate ring buffer)
...
// runtime baud-rate setter for the PIO pin5 UART (used by CRSF autobaud).
// must be called with RX disabled (TX SM may be running and idle on pull-block).
void uart_setbaudrate(uint32_t baud)
{
    float div = (float)clock_get_hz(clk_sys) / (8.0f * (float)baud);
    pio_sm_set_clkdiv(pio_uart, pio_sm_rx, div);
    pio_sm_set_clkdiv(pio_uart, pio_sm_tx, div);
    pio_sm_clkdiv_restart(pio_uart, pio_sm_rx);
    pio_sm_clkdiv_restart(pio_uart, pio_sm_tx);
}


void tPin5BridgeBase::pin5_set_protocol(uint32_t baudrate)
{
    pin5_tx_enable(); // disables RX, enables TX
    uart_setbaudrate(baudrate);
    pin5_rx_enable(); // back to RX mode
}


void tPin5BridgeBase::pin5_putbuf(uint8_t* const buf, uint16_t buflen)
{
    if (buflen > UART_TXBUFSIZE) buflen = UART_TXBUFSIZE;
