...
class tTxWifiNative : public tSerialBase
{
  ...
  private:
    ...
    void udp_setup(void)
    {
        ...
        udp_pcb = udp_new();
        if (udp_pcb != nullptr) {
            if (udp_bind(udp_pcb, IP_ADDR_ANY, WIFI_UDP_PORT) == ERR_OK) {
    ...
    void do_tx_udp(void)
    {
        if (udp_pcb == nullptr) { tx_fifo.Flush(); return; } // socket setup failed
    ...
    void tcp_setup(void)
    {
        ...
        if (l != nullptr) {
            if (tcp_bind(l, IP_ADDR_ANY, WIFI_TCP_PORT) == ERR_OK) {
                tcp_listen_pcb = tcp_listen_with_backlog(l, 1); // frees l on success
            }
            if (tcp_listen_pcb != nullptr) {
    ...
    static err_t tcp_recv_callback(void* arg, struct tcp_pcb* tpcb, struct pbuf* p, err_t err)
    {
        ...
        if (self != nullptr) {
            if (!self->rx_fifo.HasSpace(p->tot_len)) return ERR_MEM; // keep the pbuf, retried by lwIP
            struct pbuf* q = p;
            while (q != nullptr) {
                self->rx_fifo.PutBuf((char*)q->payload, q->len);
                q = q->next;
...
void wifi_init(void)
{
    ...
    uint8_t sn[RP_MCU_UID_LEN];
    mcu_uid(sn); // first 8 bytes are the unique board id, as mcu_serial_number() used to return
...
// note: a config change needs no handling here, RESTARTCONTROLLER is a full reboot
// on RP (see rp-glue.h), so this runs once per boot
void wifi_loop(void)
{
